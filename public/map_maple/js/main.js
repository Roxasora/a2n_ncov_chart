...
function fetchChoroplethData() {
    var promises = []
    promises.push($.getJSON('data/' + dateStr +'/' + dateStr + '-' + jsonHour + '-' + jsonMinute + '_2_perc10.topojson'))
    if (perc20 === undefined) {
...
function loadStats(canvasOverlay, info) {
    console.log('loading province/city data')
...
function loadLocations(canvasOverlay, info) {
    ...
    if (map.hasLayer(srcCont)) { srcCont.removeFrom(map) }
    ...
    for (var i = 0; i < locations.length; i++) {
        ...
        if (info.bounds.contains([lat, lon])) {
            ...
            if (zoom > 11) {
                ...
                // ctx.fillStyle = "rgba(200,200,80, 0.35)";
                // ctx.fillRect(dot.x, dot.y, )
                ctx.closePath()
                
                console.log('this link', link, addr)
                if (link !== '') {
                    var iconOptions = {
                        color: "rgb(0,0,100)",
                        ...
                        circleRatio: 0.03,
                        fontSize: 13,
                        cirTag: '<a href="' + link + '" target="view_window">',
                        cirTxt: '查看来源',
                        fontColor: '#1090d8'
                    }
                } else {
                ...
                srcGrp.push(new L.Marker.SVGMarker(L.latLng([lat, lon]), {
    ...
    srcCont = L.layerGroup(srcGrp).addTo(map)
}
